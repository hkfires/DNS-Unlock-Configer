...
@app.route('/api/generate_singbox_config', methods=['POST'])
def api_generate_singbox_config():
    try:
        data = request.get_json()
        if not data:
            return jsonify({"error": "无效的请求：需要 JSON 数据。"}), 400
        ...
        result_string = _generate_singbox_domain_list(selected_domains)
        return jsonify({"config": result_string})

    except Exception as e:
        print(f"Error in /api/generate_singbox_config: {e}")
        return jsonify({"error": "生成 Sing-box 规则时发生内部服务器错误。"}), 500
